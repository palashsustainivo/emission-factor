...
const fs = require('fs');
const baseUrl = process.env.BASE_URL + '/v1/ghg_protocol_category_master';
const token = process.env.USER_TOKEN;

describe('GHG Protocol Category Master API', () => {
  ...
  it('Create Reccord', async () => {
    let bodyData = {
      ghg_protocol_category_name:"Test 22",
      scope:"Scope 1",
    };
  ...
  it('Update Single Reccord', async () => {
    ...
    const response = await axios.put(baseUrl + '/85afcdbc-fb09-49e8-a6f1-f1a0fe0e4a15', bodyData, {
      headers: {
        'Authorization': `Bearer ${token}`
      },
    });
  ...
  it('Bulk Upload', async () => {
    ...
    const filePath = './SampleBulkUploadFile/sample_ghg_protocol_category_master.xlsx'; // Replace with your file path
