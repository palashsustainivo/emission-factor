...
import dbCrud from '../../models/licenseMaster/index.js';
import modelDbCrud from './dbCrud.js';
import licenseTypeModelDbCrud from '../../models/licenseTypeMaster/index.js';
import readExcel from '../../common/readExcel.js';
const { Op } = require('sequelize');
const log = require('node-file-logger');
import sanitizeHtml from 'sanitize-html';
const primaryKey = "license_id";

/**
 * Get all license master records
...
/**
 * Get a single license master record by ID
 * @param {Request} req - Express request object
 * @param {Response} res - Express response object
 */
...
/**
 * Create a new license master record
 * @param {Request} req - Express request object
 * @param {Response} res - Express response object
 */
...
/**
 * Create multiple license master records while bulk uploading
 * @param {Request} req - Express request object
 * @param {Response} res - Express response object
 */
  ...
  try {
    ...
    if(dbStoreData) {
      ...
      for(let item of dbStoreData) {
        ...
        let returnQueryData = await licenseTypeModelDbCrud.findOneReccord({license_type_name: item.license_type, is_deleted: process.env.IS_DELETED_NO});
        if(returnQueryData != null){
          modifiedRow++;
          rowItem.license_type_id = returnQueryData.license_type_id;
          modifiedData.push(rowItem);
        } else {
          let insertData = await licenseTypeModelDbCrud.create({license_type_name: item.license_type, created_by: item.created_by});
          let pushData = {
            license_type_id: insertData.license_type_id,
            url: item.url,
            created_by: item.created_by,
          };
          modifiedData.push(pushData);
          modifiedRow++;
        }
      }
      dbCrud.bulkCreate(modifiedData);
    } else {
      let sendResponse = {
        data: [],
...
/**
 * Update a license master record by ID
 * @param {Request} req - Express request object
 * @param {Response} res - Express response object
 */
...
/**
 * Delete a license master record by ID
 * @param {Request} req - Express request object
 * @param {Response} res - Express response object
 */
...
/**
 * Delete multiple license master record by ID Array
 * @param {Request} req - Express request object
 * @param {Response} res - Express response object
 */
...
/**
 * Soft Delete multiple license master record by ID Array
 * @param {Request} req - Express request object
 * @param {Response} res - Express response object
 */
